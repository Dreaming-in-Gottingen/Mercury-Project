...
// framework header, move out in the future.
#include "mediarecorder.h"

/*
 * this mp4 muxer base on Android4.4.4,
 * Android's implementation is too huge and complicated
 * compared with ts muxer,
 * which cause I dislike to transplant it!
 * But for the reason that Mercury-Project need it to
 * cover common requirements,
 * I make a lot of changes to make it run at last.
 *
 * changed history:
 * V0.1 -- no audio, only can be played by ffplay.
 */
...
status_t MPEG4Writer::addSource(const sp<MediaSource> &source) {
    ...
    // careful sp<MetaData> will disappear.
    // it only appear on it's RefCnt>=1, disappear when return.
    // OMXCodec had better keep format MetaData as member, if not, muxer must do below. 
    //bool find = source->getFormat()->findCString(kKeyMIMEType, &mime);
    sp<MetaData> meta = source->getFormat();
    const char *mime;
    bool find = meta->findCString(kKeyMIMEType, &mime);
...
void MPEG4Writer::setStartTimestampUs(int64_t timeUs) {
    ...
    CHECK_GE(timeUs, 0ll);
    Mutex::Autolock autoLock(mLock);
    ALOGI("MPEG4Writer before modify StartTimestampUs: mStartTimestampUs = %lld", (long long)mStartTimestampUs);
...
int64_t MPEG4Writer::getStartTimestampUs() {
    Mutex::Autolock autoLock(mLock);
    ALOGI("MPEG4Writer getStartTimestampUs: mStartTimestampUs = %lld", (long long)mStartTimestampUs);
...
uint32_t MPEG4Writer::getVideoChunkToWriter()
{
    Mutex::Autolock autolock(mLock);
    //CHECK_EQ(mDone, false);
    size_t size = 0;

    if(mChunkInfos.empty()) {
       return 0;
    }

    for (List<ChunkInfo>::iterator it = mChunkInfos.begin();
            it != mChunkInfos.end(); ++it) {
        //ALOGI("the number of chunckInfo is %d, audio:%d, chunck size:%d", mChunkInfos.size(),it->mTrack->isAudio(), it->mChunks.size());
        if (it->mTrack->isAudio() != true && !it->mChunks.empty() ) {// only need video chunk
            for(List<Chunk>::iterator chunckit=it->mChunks.begin();
                    chunckit != it->mChunks.end(); ++chunckit){
                if(!chunckit->mSamples.empty()) {
                    for(List<MediaBuffer *>::iterator mb = chunckit->mSamples.begin();
                            mb != chunckit->mSamples.end(); ++mb) {
                            size += (*mb)->range_length();
                    //ALOGI("the number of samples in a chunck is %d,the timestamp for the chunck: %lld ms", size, chunckit->mTimeStampUs/1000);
                    }
                }
            }
        }
...
status_t MPEG4Writer::Track::threadEntry() {
    ...
    const bool hasMultipleTracks = true;//(mOwner->numTracks() > 1);
    ...
    bool isFirstRestoreFrame = false;
    ...
    while (!mDone && (err = mSource->read(&buffer)) == OK) {
        ...
        if (mPaused && !mResumed) {
            ...
            if (!mIsAudio) {
                 mOwner->mWaitSyncFrame = true;
            }
            continue;
        }

        //If lots of video's chunk, we discard some chunk for Tcard write pressure
        if (!mIsAudio) {
            size_t size = mOwner->getVideoChunkToWriter();
            int32_t interval = 0;

            if(size > 16000000) { //more than 16MB,we will drop video frame
                ALOGI("video track drop one frame");
                buffer->release();
                buffer = NULL;
                isFirstRestoreFrame = true;
                continue;
            }

            if(isFirstRestoreFrame) {
                int32_t isSync = false;
                (buffer->meta_data())->findInt32(kKeyIsSyncFrame, &isSync);

                if(!isSync) {
                    ALOGI("video track drop not key frame");
                    buffer->release();
                    buffer = NULL;
                    continue;
                } else {
                    isFirstRestoreFrame = false;
                }
            }

            if(size > 14000000) { //more than 10MB chunk buffer to write
            ...
            if( interval > 0 ) {
                ALOGI("video chunk has %dMB bytes to write, sleep %d ms", size/1000000, interval/1000);
                usleep(interval);
            }
        }
        ...
        // why not strip start_code? I comment out this line!
        ...
        int32_t isSync = false;
        int find = meta_data->findInt32(kKeyIsSyncFrame, &isSync);
...
////////////////////////////////////////////////////////////////////////////////
        if (mStszTableEntries->count() == 0) {
            ...
            ALOGI("%s Track mStartTimestampUs = %lld",mIsAudio?"Audio":"Video",(long long)mStartTimestampUs);
        ...
        if (mResumed) {
            ...
            //skip the frames paused between lastDurationUs, otherwise the trackDuration will be not correct
            if (pausedDurationUs <  lastDurationUs) {
            ...
            ALOGV("mIsAudio: %d, isSync: %d, timestampUs: %lld, previousPausedDurationUs: %lld, mTrackDurationUs: %lld, lastDurationUs: %lld , pausedDurationUs: %lld",
                      mIsAudio, isSync, (long long)timestampUs, (long long)previousPausedDurationUs, (long long)mTrackDurationUs, (long long)lastDurationUs, (long long)pausedDurationUs);
            //the time between resume start and the first I frame will be recgnized as paused time
            //after resume, the first I frame be the paused ending time
            if (!mIsAudio) {
                if (!isSync) {
                    copy->release();
                    copy = NULL;
                    mMdatSizeBytes -= sampleSize;
                    mSource->requestIDRFrame();
                    continue;
                }
                mOwner->mWaitSyncFrame = false;
            } else {
                if(mOwner->mWaitSyncFrame){
                    copy->release();
                    copy = NULL;
                    mMdatSizeBytes -= sampleSize;
                    continue;
        ...
        if (currDurationTicks < 0ll) {
           ...
           // return UNKNOWN_ERROR;  //ignore this for write B Frames
        }
...
int32_t MPEG4Writer::Track::getStartTimeOffsetScaledTime() const {
    ...
    int64_t moovStartTimeUs = mOwner->getStartTimestampUs();
    ALOGI("%s Track getStartTimeOffsetScaledTime: mStartTimestampUs = %lld moovStartTimeUs = %lld",
        mIsAudio?"Audio":"video",(long long)mStartTimestampUs,(long long)moovStartTimeUs);
    if (mStartTimestampUs != moovStartTimeUs) {

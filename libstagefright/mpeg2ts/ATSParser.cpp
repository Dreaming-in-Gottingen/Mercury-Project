...
static bool is_null_packet = false;
...
bool ATSParser::isNULLPacket() {
    return is_null_packet;
}
...
status_t ATSParser::Stream::parse(
    ...
    if (mExpectedContinuityCounter >= 0
        ...
        ALOGI("discontinuity on stream pid 0x%04x.Try to continue demux.donot drop it.", mElementaryPID);
/*
        mPayloadStarted = false;
        mBuffer->setRange(0, 0);
        mExpectedContinuityCounter = -1;
...
status_t ATSParser::feedTSPacket(const void *data, size_t size, SyncEvent *event) {
    //ALOGD("feedTSPacket");
...
status_t ATSParser::parsePID(
    ...
    if (sectionIndex >= 0) {
        ...
        if (payload_unit_start_indicator) {
            if (!section->isEmpty()) {
                ALOGI("parsePID encounters payload_unit_start_indicator when section is not empty");
                section->clear();
...
status_t ATSParser::parseTS(ABitReader *br, SyncEvent *event) {
    ...
    // TS file has too much of the head.
    if(PID == 0x1FFF){
        ALOGV("PID = 0x%04x is NULL Packet", PID);
        is_null_packet = true;
        return OK;
    }
    ...
    if (adaptation_field_control == 1 || adaptation_field_control == 3) {
        err = parsePID(br, PID, continuity_counter, payload_unit_start_indicator, event);
    }
    ++mNumTSPacketsParsed;

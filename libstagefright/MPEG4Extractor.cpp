...
*/
    ...
    if (numChannels == 0) {
        numChannels = 2;  //for some special file
//        return ERROR_UNSUPPORTED;
    }
...
status_t MPEG4Source::read(
    ...
    *out = NULL;
    ...
    if (mBuffer == NULL) {
        ...
        if (err != OK) {
            if (err == ERROR_OUT_OF_RANGE) {
                // read metadata near the end of the stream would cause this ERROR_OUT_OF_RANGE error.
                // if this, ending normally.
                err = ERROR_END_OF_STREAM;
            }
            return err;
        }
        ...
        *out = clone;
        ...
        if (usesDRM) {
            ...
            while (srcOffset < size) {
                ...
                {
                    ALOGE("Video is malformed: Line: %d",__LINE__);
                    ALOGE("mCurrentSampleIndex: %d,offset: %lld,size: %d ", \
                           mCurrentSampleIndex,offset,(uint32_t)size);
                    ALOGE("srcOffset: %d, mNALLengthSize: %d,last nalLength:%d ", \
                           srcOffset, (uint32_t)mNALLengthSize,(uint32_t)nalLength);
                ...
                if (isMalFormed) {
                    ...
                    ALOGE("mCurrentSampleIndex: %d,offset: %lld,size: %d ",\
                           mCurrentSampleIndex, offset, (uint32_t)size);
                    ALOGE("srcOffset: %d,mNALLengthSize: %d,nalLength: %d ",\
                          (uint32_t)srcOffset, (uint32_t)mNALLengthSize,(uint32_t)nalLength);
                    ALOGE("dump addr for nalLength:0x%x %x %x %x,0x%x %x %x %x",\
                           mSrcBuffer[srcOffset],  mSrcBuffer[srcOffset+1], \
                           mSrcBuffer[srcOffset+2],mSrcBuffer[srcOffset+3], \
                           mSrcBuffer[srcOffset+4],mSrcBuffer[srcOffset+5], \
                           mSrcBuffer[srcOffset+6],mSrcBuffer[srcOffset+7]);
                    mBuffer->release();
                    mBuffer = NULL;
                    return ERROR_MALFORMED;
                }

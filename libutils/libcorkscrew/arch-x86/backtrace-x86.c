...
static uintptr_t find_fde(const memory_t* memory,
    ...
    */
    if (!read_dwarf(memory, eh_frame_hdr, (uint32_t*)(&eh_hdr_info.eh_frame_ptr), eh_hdr_info.eh_frame_ptr_enc, &c)) return 0;
    if (!read_dwarf(memory, eh_frame_hdr, (uint32_t*)(&eh_hdr_info.fde_count), eh_hdr_info.fde_count_enc, &c)) return 0;
    ...
    while (low <= high) {
        ...
        if (!read_dwarf(memory, eh_frame_hdr, (uint32_t*)(&start), eh_hdr_info.fde_table_enc, &entry)) return 0;
        if (pc <= start) {
            high = mid - 1;
        } else {
            low = mid + 1;
    ...
    c += high * 8;
    if (!read_dwarf(memory, eh_frame_hdr, (uint32_t*)(&start), eh_hdr_info.fde_table_enc, &c)) return 0;
    if (!read_dwarf(memory, eh_frame_hdr, (uint32_t*)(&fde), eh_hdr_info.fde_table_enc, &c)) return 0;
...
static bool execute_fde(const memory_t* memory,
    ...
    */
    ...
    if (!try_get_word(memory, fde + 4, (uint32_t*)(&cie_offset))) {
        return false;
    }
    if (cie_offset == 0) {
        ...
        if (!try_get_word(memory, cie + 4, (uint32_t*)(&cie_offset))) {
           return false;
        }
        if (cie_offset != 0) {

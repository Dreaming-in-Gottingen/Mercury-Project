...
void testHandRelease()
{
    ...
    ALOGD("function:%s", __FUNCTION__);
    sp<DerivedTest> sp_dt = new DerivedTest();
    ALOGD("0000. after new, strongCnt: [%d]", sp_dt->getStrongCount());
    sp_dt->incStrong(NULL);
    ALOGD("1111. after incStrong, strongCnt: [%d]", sp_dt->getStrongCount());
    sp_dt->decStrong(sp_dt.get());
    ALOGD("2222. after decStrong, strongCnt: [%d]", sp_dt->getStrongCount());
    //sp_dt->decStrong(sp_dt.get()); // may coredump on some platforms
    //sp_dt.clear();
